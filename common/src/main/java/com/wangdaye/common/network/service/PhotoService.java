...
public class PhotoService {
    ...
    public void requestCuratePhotos(@ListPager.PageRule int page, @ListPager.PerPageRule int per_page,
    ...
    public void requestRandomPhotos(List<Integer> collectionIdList,
        ...
        api.getRandomPhotos(idString, featured, username, query, orientation, ListPager.DEFAULT_PER_PAGE)
                .compose(SchedulerTransformer.create())
                .subscribe(new ObserverContainer<>(compositeDisposable, observer));
    }
    ...
    public List<Photo> requestRandomPhotos(@Nullable List<Integer> collectionIdList, Boolean featured,
        ...
        try {
            return api.callRandomPhotos(
                    collections.toString(), featured, username, query, orientation, ListPager.DEFAULT_PER_PAGE
            ).execute().body();
        } catch (IOException e) {
            e.printStackTrace();
        }
