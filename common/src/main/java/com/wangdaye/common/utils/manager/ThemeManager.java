...
public class ThemeManager {
    ...
    private ThemeManager(Context context) {
        ...
        switch (ComponentFactory.getSettingsService().getAutoNightMode()) {
            case SettingsService.DAY_NIGHT_MODE_AUTO:
                setLightTheme(context, !isAutoNight(nightStartTime, nightEndTime));
                break;

            case SettingsService.DAY_NIGHT_MODE_FOLLOW_SYSTEM:
    ...
    public void setLightTheme(Context context, boolean lightTheme) {
        ...
        context.getSharedPreferences(PREFERENCE_NAME, Context.MODE_PRIVATE)
                .edit()
                .putBoolean(KEY_LIGHT_THEME, lightTheme)
                .apply();
    }
